...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        // ✅ Lấy lại category đang được lọc từ session
        Object categoryFilter = session.getAttribute("categoryFilter");

        ArrayList<Models.Paging> pages;

        if (categoryFilter == null || "all".equals(categoryFilter)) {
            pages = d.getDataForPage(d.getAllProduct());
        } else {
            int categoryID = (int) categoryFilter;
            pages = d.getDataForPage(d.getProductByCategory(categoryID));
        }
