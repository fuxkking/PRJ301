...
public class ProductDAO extends DBContext {
    ...
    public ArrayList<Product> getTop4BestSeller() {
        ArrayList<Product> products = new ArrayList<>();
        try {
            String sql = "SELECT TOP (4) * FROM [products] WHERE productID NOT IN (SELECT productID FROM HiddenProducts) ORDER BY sold DESC";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Category category = getCategoryByID(rs.getInt("categoryID"));
    ...
    public ArrayList<Product> searchProuctsByName(String name) {
        ArrayList<Product> products = new ArrayList<>();
        try {
            String sql = "SELECT * FROM [products] WHERE name LIKE N'%" + name + "%' AND productID NOT IN (SELECT productID FROM HiddenProducts)";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Category category = getCategoryByID(rs.getInt("categoryID"));
